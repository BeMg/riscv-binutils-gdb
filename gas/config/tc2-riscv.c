...
#define RELAX_BRANCH_ENCODE(uncond, rvc, length)                               \
  ((relax_substateT)(0xc0000000 | ((uncond) ? 1 : 0) | ((rvc) ? 2 : 0) |       \
                     ((length) << 2)))
...
#define OPCODE_MATCHES(OPCODE, OP) (((OPCODE)&MASK_##OP) == MATCH_##OP)
...
static void create_insn(struct riscv_cl_insn *insn,
...
static void add_relaxed_insn(struct riscv_cl_insn *insn, int max_chars, int var,
                             relax_substateT subtype, symbolS *symbol,
                             offsetT offset) {
  frag_grow(max_chars);
  move_insn(insn, frag_now, frag_more(0) - frag_now->fr_literal);
  frag_var(rs_machine_dependent, max_chars, var, subtype, symbol, offset, NULL);
}
...
static unsigned relaxed_branch_length(fragS *fragp, asection *sec, int update) {
  ...
  if (fragp->fr_symbol != NULL && S_IS_DEFINED(fragp->fr_symbol) &&
      !S_IS_WEAK(fragp->fr_symbol) && sec == S_GET_SEGMENT(fragp->fr_symbol)) {
...
enum reg_class { RCLASS_GPR, RCLASS_FPR, RCLASS_CSR, RCLASS_MAX, RCLASS_VPR };
...
static void hash_reg_names(enum reg_class class, const char *const names[],
                           unsigned n) {
  unsigned i;
...
static bfd_boolean reg_lookup(char **s, enum reg_class class,
                              unsigned int *regnop) {
  char *e;
  char save_c;
  int reg = -1;
...
static bfd_boolean arg_lookup(char **s, const char *const *array, size_t size,
                              unsigned *regnop) {
...
/* For consistency checking, verify that all bits are specified either
   ...
   operand list.  */
static bfd_boolean validate_riscv_insn(const struct riscv_opcode *opc) {
  ...
  if ((used_bits & opc->match) != (opc->match & required_bits)) {
    as_bad(_("internal: bad RISC-V opcode (mask error): %s %s"), opc->name,
           opc->args);
    return FALSE;
  }
...
#define USE_BITS(mask, shift) (used_bits |= ((insn_t)(mask) << (shift)))
  while (*p)
    ...
    case 'C': /* RVC */
      switch (c = *p++) {
      case 'a':
        used_bits |= ENCODE_RVC_J_IMM(-1U);
        break;
      case 'c':
        break; /* RS1, constrained to equal sp */
      case 'i':
        used_bits |= ENCODE_RVC_SIMM3(-1U);
        break;
      case 'j':
        used_bits |= ENCODE_RVC_IMM(-1U);
        break;
      case 'o':
        used_bits |= ENCODE_RVC_IMM(-1U);
        break;
      case 'k':
        used_bits |= ENCODE_RVC_LW_IMM(-1U);
        break;
      case 'l':
        used_bits |= ENCODE_RVC_LD_IMM(-1U);
        break;
      case 'm':
        used_bits |= ENCODE_RVC_LWSP_IMM(-1U);
        break;
      case 'n':
        used_bits |= ENCODE_RVC_LDSP_IMM(-1U);
        break;
      case 'p':
        used_bits |= ENCODE_RVC_B_IMM(-1U);
        break;
      case 's':
      ...
      case 'v':
        ...
        break;
      case 'w':
        break; /* RS1S, constrained to equal RD */
      case 'x':
        break; /* RS2S, constrained to equal RD */
      case 'K':
        used_bits |= ENCODE_RVC_ADDI4SPN_IMM(-1U);
        break;
      case 'L':
        used_bits |= ENCODE_RVC_ADDI16SP_IMM(-1U);
        break;
      case 'M':
        used_bits |= ENCODE_RVC_SWSP_IMM(-1U);
        break;
      case 'N':
        used_bits |= ENCODE_RVC_SDSP_IMM(-1U);
        break;
      case 'U':
        break; /* RS1, constrained to equal RD */
      case 'V':
    ...
    case 'S':
      ...
      break;
    case 'U':
      USE_BITS(OP_MASK_RS1, OP_SH_RS1); /* fallthru */
    case 'T':
    ...
    case 'j':
      ...
      break;
    case 'a':
      used_bits |= ENCODE_UJTYPE_IMM(-1U);
      break;
    case 'p':
      used_bits |= ENCODE_SBTYPE_IMM(-1U);
      break;
    case 'q':
...
#undef USE_BITS
  if (used_bits != required_bits) {
    as_bad(_("internal: bad RISC-V opcode (bits 0x%lx undefined): %s %s"),
           ~(unsigned long)(used_bits & required_bits), opc->name, opc->args);
    return FALSE;
  }
  return TRUE;
}
...
static insn_t riscv_apply_const_reloc(bfd_reloc_code_real_type reloc_type,
                                      bfd_vma value) {
  switch (reloc_type) {
  case BFD_RELOC_32:
    return value;
...
static void normalize_constant_expr(expressionS *ex) {
  ...
  if ((ex->X_op == O_constant || ex->X_op == O_symbol) &&
    ...
    ex->X_add_number =
        (((ex->X_add_number & 0xffffffff) ^ 0x80000000) - 0x80000000);
}
...
static void pcrel_store(int srcreg, int tempreg, expressionS *ep,
                        const char *lo_insn, bfd_reloc_code_real_type hi_reloc,
                        bfd_reloc_code_real_type lo_reloc) {
...
static void load_const(int reg, expressionS *ep) {
  ...
  lower.X_add_number =
      (int32_t)ep->X_add_number << (32 - shift) >> (32 - shift);
...
static void macro(struct riscv_cl_insn *ip, expressionS *imm_expr,
  ...
  case M_LLA:
    ...
    if (imm_expr->X_op == O_constant)
      ...
      pcrel_load(rd, rd, imm_expr, LOAD_ADDRESS_INSN, BFD_RELOC_RISCV_GOT_HI20,
                 BFD_RELOC_RISCV_PCREL_LO12_I);
    else /* Local PIC symbol, or any non-PIC symbol */
      pcrel_load(rd, rd, imm_expr, "addi", BFD_RELOC_RISCV_PCREL_HI20,
                 BFD_RELOC_RISCV_PCREL_LO12_I);
    break;

  case M_LA_TLS_GD:
    pcrel_load(rd, rd, imm_expr, "addi", BFD_RELOC_RISCV_TLS_GD_HI20,
               BFD_RELOC_RISCV_PCREL_LO12_I);
    break;
  ...
  case M_LBU:
    pcrel_load(rd, rd, imm_expr, "lbu", BFD_RELOC_RISCV_PCREL_HI20,
               BFD_RELOC_RISCV_PCREL_LO12_I);
    break;

  case M_LH:
    pcrel_load(rd, rd, imm_expr, "lh", BFD_RELOC_RISCV_PCREL_HI20,
               BFD_RELOC_RISCV_PCREL_LO12_I);
    break;

  case M_LHU:
    pcrel_load(rd, rd, imm_expr, "lhu", BFD_RELOC_RISCV_PCREL_HI20,
               BFD_RELOC_RISCV_PCREL_LO12_I);
    break;

  case M_LW:
    pcrel_load(rd, rd, imm_expr, "lw", BFD_RELOC_RISCV_PCREL_HI20,
               BFD_RELOC_RISCV_PCREL_LO12_I);
    break;

  case M_LWU:
    pcrel_load(rd, rd, imm_expr, "lwu", BFD_RELOC_RISCV_PCREL_HI20,
               BFD_RELOC_RISCV_PCREL_LO12_I);
    break;

  case M_LD:
    pcrel_load(rd, rd, imm_expr, "ld", BFD_RELOC_RISCV_PCREL_HI20,
               BFD_RELOC_RISCV_PCREL_LO12_I);
    break;

  case M_FLW:
    pcrel_load(rd, rs1, imm_expr, "flw", BFD_RELOC_RISCV_PCREL_HI20,
               BFD_RELOC_RISCV_PCREL_LO12_I);
    break;

  case M_FLD:
    pcrel_load(rd, rs1, imm_expr, "fld", BFD_RELOC_RISCV_PCREL_HI20,
               BFD_RELOC_RISCV_PCREL_LO12_I);
    break;

  case M_SB:
    pcrel_store(rs2, rs1, imm_expr, "sb", BFD_RELOC_RISCV_PCREL_HI20,
                BFD_RELOC_RISCV_PCREL_LO12_S);
    break;

  case M_SH:
    pcrel_store(rs2, rs1, imm_expr, "sh", BFD_RELOC_RISCV_PCREL_HI20,
                BFD_RELOC_RISCV_PCREL_LO12_S);
    break;

  case M_SW:
    pcrel_store(rs2, rs1, imm_expr, "sw", BFD_RELOC_RISCV_PCREL_HI20,
                BFD_RELOC_RISCV_PCREL_LO12_S);
    break;

  case M_SD:
    pcrel_store(rs2, rs1, imm_expr, "sd", BFD_RELOC_RISCV_PCREL_HI20,
                BFD_RELOC_RISCV_PCREL_LO12_S);
    break;

  case M_FSW:
    pcrel_store(rs2, rs1, imm_expr, "fsw", BFD_RELOC_RISCV_PCREL_HI20,
                BFD_RELOC_RISCV_PCREL_LO12_S);
    break;

  case M_FSD:
    pcrel_store(rs2, rs1, imm_expr, "fsd", BFD_RELOC_RISCV_PCREL_HI20,
                BFD_RELOC_RISCV_PCREL_LO12_S);
    break;
...
static const struct percent_op_match percent_op_rtype[] = {
    {"%tprel_add", BFD_RELOC_RISCV_TPREL_ADD}, {0, 0}};
...
static size_t my_getSmallExpression(expressionS *ep,
                                    bfd_reloc_code_real_type *reloc, char *str,
                                    const struct percent_op_match *percent_op) {
  size_t reloc_index;
...
static bfd_boolean riscv_handle_implicit_zero_offset(expressionS *expr,
                                                     const char *s) {
...
static const char *riscv_ip(char *str, struct riscv_cl_insn *ip,
    ...
    for (args = insn->args;; ++args) {
      ...
      case 'C': /* RVC */
        ...
        case 'c': /* RS1, constrained to equal sp.  */
          if (!reg_lookup(&s, RCLASS_GPR, &regno) || regno != X_SP)
            break;
          continue;
        case '>':
        ...
        case '<':
          if (my_getSmallExpression(imm_expr, imm_reloc, s, p) ||
              ...
              imm_expr->X_add_number <= 0 || imm_expr->X_add_number >= 32)
            break;
          ip->insn_opcode |= ENCODE_RVC_IMM(imm_expr->X_add_number);
          goto rvc_imm_done;
        case 'i':
        ...
        case 'v':
          if (my_getSmallExpression(imm_expr, imm_reloc, s, p) ||
              ...
              ((int32_t)imm_expr->X_add_number != imm_expr->X_add_number))
            break;
          imm_expr->X_add_number =
      ...
      case 'I':
        my_getExpression(imm_expr, s);
        if (imm_expr->X_op != O_big && imm_expr->X_op != O_constant)
          break;
...
struct option md_longopts[] = {{"march", required_argument, NULL, OPTION_MARCH},
...
static void riscv_set_abi(unsigned new_xlen, enum float_abi new_float_abi) {
  abi_xlen = new_xlen;
...
void md_apply_fix(fixS *fixP, valueT *valP, segT seg ATTRIBUTE_UNUSED) {
  ...
  case BFD_RELOC_RISCV_LO12_S:
    bfd_putl32(
        riscv_apply_const_reloc(fixP->fx_r_type, *valP) | bfd_getl32(buf), buf);
...
void riscv_pre_output_hook(void) {
  const frchainS *frch;
  const asection *s;
...
static void s_dtprel(int bytes) {
  ...
  fix_new_exp(frag_now, p - frag_now->fr_literal, bytes, &ex, FALSE,
              (bytes == 8 ? BFD_RELOC_RISCV_TLS_DTPREL64
...
bfd_boolean riscv_frag_align_code(int n) {
  ...
  fix_new_exp(frag_now, nops - frag_now->fr_literal, 0, &ex, FALSE,
...
int riscv_relax_frag(asection *sec, fragS *fragp,
...
static void md_convert_frag_branch(fragS *fragp) {
  ...
  if (RELAX_BRANCH_RVC(fragp->fr_subtype)) {
    ...
    case 2:
      ...
      fixp = fix_new_exp(fragp, buf - (bfd_byte *)fragp->fr_literal, 2, &exp,
                         FALSE, reloc);
      buf += 2;
      goto done;
  ...
  jump:
    /* Jump to the target.  */
    fixp = fix_new_exp(fragp, buf - (bfd_byte *)fragp->fr_literal, 4, &exp,
  ...
  case 4:
    reloc = RELAX_BRANCH_UNCOND(fragp->fr_subtype) ? BFD_RELOC_RISCV_JMP
                                                   : BFD_RELOC_12_PCREL;
    fixp = fix_new_exp(fragp, buf - (bfd_byte *)fragp->fr_literal, 4, &exp,
                       FALSE, reloc);
    buf += 4;
  ...
  gas_assert(buf ==
             (bfd_byte *)fragp->fr_literal + fragp->fr_fix + fragp->fr_var);
